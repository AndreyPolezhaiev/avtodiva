...
public class FreeSlotsTableModel extends AbstractTableModel {
    ...
    public void sortByColumn(int columnIndex, boolean ascending) {
        ...
        List<SlotSelectionPair> pairs = new ArrayList<>(slots.size());
        for (int i = 0; i < slots.size(); i++) {
            pairs.add(new SlotSelectionPair(slots.get(i), selected.get(i)));
        }

        // 2. Сортируем пары по компаратору слотов
        Comparator<SlotSelectionPair> pairComparator = Comparator.comparing(SlotSelectionPair::getSlot, slotComparator);
